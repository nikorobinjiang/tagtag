    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-dropdown>
            <el-button type="text" size="small" class="el-dropdown-link" :disabled="scope.row.pkg_status_text !== '打包完成' && xStore.data.gameTypeWXMP !== scope.row.game_type">
              更多操作
            </el-button>
            <el-dropdown-menu slot="dropdown">
              <template v-if="xStore.data.gameTypeWXMP === scope.row.game_type">
                  <el-button type="text" size="small" v-clipboard="{ text: scope.row.wxmp_tracking }">复制追踪参数</el-button>
              </template>
              <template v-else>
                <el-dropdown-item v-if="xStore.data.gameTypeH5 === scope.row.game_type"><a :href="scope.row.shell_url" target="_blank">打开链接</a></el-dropdown-item>
                <el-dropdown-item v-else><el-button type="text" v-newtab="scope.row.apk_addr" target="_blank">下载</el-button></el-dropdown-item>
                <el-dropdown-item>
                  <el-button type="text" size="small" v-clipboard="{ text: scope.row.copy_link_content }">复制链接</el-button>
                </el-dropdown-item>

                <el-dropdown-item v-if="xStore.data.gameTypeH5 === scope.row.game_type">
                  <el-button type="text" v-newtab="scope.row.apk_addr" target="_blank">壳包地址获取</el-button>
                </el-dropdown-item>
                ...
                <el-dropdown-item>
                  <el-button type="text" size="small" @click="handleRePkg(scope.$index, scope.row)">重新打包</el-button>
                </el-dropdown-item>

                <el-dropdown-item v-if="scope.row.bi_url">
                  <el-button type="text" v-newtab="scope.row.bi_url" target="_blank">查看数据</el-button>
                </el-dropdown-item>
              </template>
...
export default {
  ...
  methods: {
    ...
    handleTableSelectable(row, index) {
      if (["Toutiao", "Uchc", "Uchcv2"].indexOf(row.distribute) === -1) {
        return false;
    ...
    handleExeclBtn() {
      ...
      window.open(this.excel_info.url + '?' + queryString.stringify(get(this.excel_info, 'query')))
    }
  },
  watch: {},
  computed: {
